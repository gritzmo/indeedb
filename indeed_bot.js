...
const JOB_QUEUE_PATH = path.resolve(__dirname, 'job_queue.json');
// Optional: connect to an existing Chrome instance via remote debugging
// Start Chrome manually with: chrome.exe --remote-debugging-port=9222
const REMOTE_CHROME_URL = 'http://localhost:9222';
...
function loadJobQueue() {
  if (!fs.existsSync(JOB_QUEUE_PATH)) return [];
  try {
    return JSON.parse(fs.readFileSync(JOB_QUEUE_PATH, 'utf8'));
  } catch (_) {
    return [];
  }
}
...
function saveJobQueue(queue) {
  fs.writeFileSync(JOB_QUEUE_PATH, JSON.stringify(queue, null, 2));
}

// Attempt to connect to a running Chrome instance via remote debugging.
// If it fails, fall back to launching a new browser.
async function launchOrConnectBrowser() {
  try {
    const browser = await puppeteer.connect({ browserURL: REMOTE_CHROME_URL });
    console.log(`Connected to Chrome at ${REMOTE_CHROME_URL}`);
    return browser;
  } catch (_) {
    console.log('Could not connect to existing Chrome. Launching new instance.');
    return puppeteer.launch({ headless: false, args: ['--start-maximized'], defaultViewport: null });
  }
}
...
async function main() {
  ...
  let queue = loadJobQueue();
  let count = 0;

  // Process any queued jobs from the Python bot first
  while (queue.length && count < cfg.max_applications) {
